...
BOOL CSketcherView::OnPreparePrinting(CPrintInfo* pInfo)
{
	...
	printData->m_DocRefPoint = docExtent.TopLeft(); //Save the document reference point
	printData->m_DocTitle = pDoc->GetTitle(); //Save the document filename


	//Calculate how many printed page widths are required
	//to accomodate the width of the document
...
void CSketcherView::MoveElement(CClientDC& aDC, const CPoint& point)
{
	...
	if (m_pSelected)
	{
		...
		{	//This is not text so use SetROP2 method
...
void CSketcherView::OnPrint(CDC* pDC, CPrintInfo* pInfo)
{
	...
	CString str;
	str.Format(_T("Page %u of %u"), pInfo->m_nCurPage);
	...
	int xOrg{ static_cast<int>(p->m_DocRefPoint.x + p->printWidth*((pInfo->m_nCurPage - 1) %
		(p->m_nWidths))) };
	int yOrg{ static_cast<int>(p->m_DocRefPoint.y + p->printLength *((pInfo->m_nCurPage - 1) /
		(p->m_nWidths))) };
	
	//Calculate offset to center drawing area on page as positive value
	...
	//Change window origin to correspond to current page and save the old origin
	...
	pDC->SetWindowOrg(OldOrg); //Restore the old origin
